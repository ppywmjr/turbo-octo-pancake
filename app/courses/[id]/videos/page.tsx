...
function VideoGridSkeleton() {
  return (
    <section className="flex flex-col gap-6">
      <div className="h-8 w-20 bg-[var(--color-zinc-200)] dark:bg-[var(--color-zinc-700)] rounded animate-pulse" />
...
async function VideoGrid({ courseId }: { courseId: string }) {
  ...
  if (videos.length === 0) {
    return <p className="text-[var(--color-zinc-500)] dark:text-[var(--color-zinc-400)] text-center">No videos found for this course.</p>
  }
